...
from vae_all.utils import paramUtil
...
def arg_parse(is_train=False):
    ...
    parser.add_argument("--vae_name", type=str, default="train_vae_on_MB_rep_try_1", help="Name of the vae model.")
    parser.add_argument("--seed", default=1234, type=int)
    parser.add_argument("--gpu_id", type=int, default=0, help="GPU id")

    ## dataloader
    parser.add_argument("--dataset_name_vae", type=str, default="t2m", help="dataset directory", choices=["t2m", "kit", "humanml"])
    ...
    parser.add_argument("--checkpoints_dir", type=str, default="vae_all/checkpoints", help="models are saved here")
    ...
    if opt.dataset_name_vae == 'humanml':
        opt.dataset_name_vae = 't2m'

    opt.save_root = pjoin(opt.checkpoints_dir, opt.dataset_name_vae, opt.name)
    opt.model_dir = pjoin(opt.save_root, 'model')
    opt.eval_dir = pjoin(opt.save_root, 'animation')
    opt.log_dir = pjoin('vae_all','./log', opt.dataset_name_vae, opt.name)

    os.makedirs(opt.model_dir, exist_ok=True)
    os.makedirs(opt.eval_dir, exist_ok=True)
    os.makedirs(opt.log_dir, exist_ok=True)

    if opt.dataset_name_vae == "t2m":
    ...
    if is_train:
        ...
        # save to the disk
        expr_dir = os.path.join(opt.checkpoints_dir, opt.dataset_name_vae, opt.name)
