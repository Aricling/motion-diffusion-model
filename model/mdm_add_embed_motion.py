...
class MDM(nn.Module):
    def __init__(self, modeltype, njoints, nfeats, num_actions, translation, pose_rep, glob, glob_rot,
        ...
        if self.cond_mode != 'no_cond':
            if 'text' in self.cond_mode:
                ...
                self.embed_motion = nn.Linear(self.clip_dim, self.latent_dim)
    ...
    def forward(self, x, timesteps, y=None):
        ...
        if 'text' in self.cond_mode:
            ...
            text_info = self.embed_text(self.mask_cond(enc_text[:,:1,:], force_mask=force_mask))
            motion_info = self.embed_motion(enc_text[:,1:,:])
            text_emb = torch.cat((text_info, motion_info), axis=1)  # 这里还是把mask只给到了text_cls
            if self.emb_policy == 'add':
