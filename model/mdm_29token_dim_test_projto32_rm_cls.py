    ...
    def forward(self, x, timesteps, y=None):
        ...
        if 'text' in self.cond_mode:
            ...
            if type(ori_bert_out) == tuple:
                ...
                ori_out, pred_motion_tokens = self.extended_proj_model(ori_bert_out_emb_projed)    ## 这里的ori_out其实也是做了self attn，可以试试看用不用,使用原来的ori_CLIP_cls_emb
                ...
                if z_config.get_diy_config().training_input.use_cls_token:  # 当前使用 BERT 的 gt
                    ...
                    bs = pred_motion_tokens_2.shape[1]
                    pred_branch = pred_motion_tokens_2
...
class ExtendedTransformerEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        combined = torch.cat([x, learned], dim=0)          # [1+50+28=79, 64, 768]

        # Transformer 编码
        output = self.transformer_encoder(combined)                 # [79, 64, 768]

        # 分离出各部分
        ori_output = output[:seq_len, :, :]     # [50, 64, 768]
        new_output = output[seq_len:, :, :]      # [28, 64, 768]
