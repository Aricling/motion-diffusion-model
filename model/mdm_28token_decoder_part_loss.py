                ...
                if z_config.get_diy_config().training_input.use_cls_token:  # 当前使用 BERT 的 gt
                    ...
                    if gt_3d_rep is not None:
                        ...
                        if not eval_time:
                            if mask_mode == 1:
                                # ---- 时间维度 mask ----
                                time_mask = torch.rand(bs, T, device=gt_3d_rep.device) < mask_prob  # (bs, 4)
                                time_mask = time_mask[:, :, None, None]  # (bs, 4, 1, 1)
                                gt_3d_rep = gt_3d_rep.masked_fill(time_mask, 0.0)

                            elif mask_mode == 2:
                                # ---- 关节维度 mask ----
                                joint_mask = torch.rand(bs, J, device=gt_3d_rep.device) < mask_prob  # (bs, 7)
                                joint_mask = joint_mask[:, None, :, None]  # (bs, 1, 7, 1)
                                gt_3d_rep = gt_3d_rep.masked_fill(joint_mask, 0.0)

                            elif mask_mode == 3:
                                # ---- token 级 mask ----
                                token_mask = torch.rand(bs, T, J, device=gt_3d_rep.device) < mask_prob  # (bs, 4, 7)
                                token_mask = token_mask[:, :, :, None]  # (bs, 4, 7, 1)
