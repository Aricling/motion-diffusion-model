...
class MDM(nn.Module):
    def __init__(self, modeltype, njoints, nfeats, num_actions, translation, pose_rep, glob, glob_rot,
        ...
        #             self.MB_rep_proj = nn.Linear(self.clip_dim, self.latent_dim)
        #             self.lora_clip_MB_rep_proj = nn.Linear(self.clip_dim, self.latent_dim)
        num_layers = z_config.get_diy_config().model.encoder_or_decoder_layers
        if int(z_config.get_diy_config().VAE_emb_shape_gt.pooled_dim) !=4:
            self.extended_proj_model = ExtendedTransformerEncoder(embed_dim=self.diy_latent_dim, num_heads=8, num_layers=num_layers, add_tokens=7*int(z_config.get_diy_config().VAE_emb_shape_gt.pooled_dim))
        else:
            self.extended_proj_model = ExtendedTransformerEncoder(embed_dim=self.diy_latent_dim, num_heads=8, num_layers=num_layers, add_tokens=28)
        ...
        if 'text' in self.cond_mode:
            ...
            if type(ori_bert_out) == tuple:
                ...
                if z_config.get_diy_config().training_input.use_cls_token:  # 当前使用 BERT 的 gt
                    ...
                    pred_branch = pred_motion_tokens
...
class ExtendedTransformerEncoder(nn.Module):
    def __init__(self, embed_dim=768, num_heads=8, num_layers=1, add_tokens=28, 
                 add_token_position='append'):
        """
        add_token_position: 'prepend' or 'append'
        """
        super().__init__()
        self.embed_dim = embed_dim
        self.add_tokens = add_tokens
        self.add_token_position = add_token_position

        # learnable global tokens (e.g., motion queries)
    ...
    def forward(self, x, src_key_padding_mask=None):
        """
        x: input features (S, B, D)
        """
        seq_len, batch_size, _ = x.size()

        # add learnable tokens
        learnable_tokens = self.learnable_tokens.expand(-1, batch_size, -1)

        if self.add_token_position == 'prepend':
            x = torch.cat([learnable_tokens, x], dim=0)  # (T_add + S, B, D)
        else:
            x = torch.cat([x, learnable_tokens], dim=0)  # (S + T_add, B, D)

        # Encoder self-attends over all tokens
        out = self.transformer_encoder(x, src_key_padding_mask=src_key_padding_mask)

        # output of learnable tokens only
        if self.add_token_position == 'prepend':
            learnable_out = out[:self.add_tokens]
        else:
            learnable_out = out[-self.add_tokens:]

        return learnable_out  # (T_add, B, D), (S_total, B, D)
