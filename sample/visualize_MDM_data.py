...
def main(motion_name_list, args=None):
    # if args is None:
    ...
    #                            get_rotations_back=False)    ## 其实因为是xyz表示，所以啥也没干直接return了
    ...
    motion_basepath="/data/motion/HumanML3D/new_joints"
    text_base_path="/data/motion/HumanML3D/texts"
    out_path="/home/mengqing/usr/motion-diffusion-model/save/models_to_upload/humanml_trans_dec_512_bert/gt_visualize"
    animations = np.empty(shape=(10, 1), dtype=object)
    rep=0
    skeleton =paramUtil.t2m_kinematic_chain
    fps=20
    all_lengths=[]
    for sample_i, motion_name in enumerate(motion_name_list):
        motion_path = os.path.join(motion_basepath, motion_name+".npy")
        ...
        with open(os.path.join(text_base_path, motion_name+".txt"), 'r') as fr:
            caption = fr.read().strip()
        animation_save_path = os.path.join(out_path, motion_name+".mp4")
        gt_frames = np.arange(0)
        animations[sample_i, rep] = plot_3d_motion(animation_save_path, 
                                                        skeleton, motion, dataset="humanml", title=caption, 
                                                        fps=fps, gt_frames=gt_frames)
    all_file_template = 'samples_{:02d}_to_{:02d}.mp4'
    save_multiple_samples(out_path, {'all': all_file_template}, animations, fps, max(list(all_lengths)))
...
if __name__ == "__main__":
    motion_name_list=["001473","002647","002707","010658","013468","M004411","M005468","M007893","M013911"]
    main(motion_name_list)